...
- [x] At least four accounts across two or three workspaces exist in
...
- [x] Quit the app normally; a saved `sessions.toml` written during these
...
- [x] With a screen: `Ctrl`+`?` while a game page has keyboard focus opens the
      ...
      grid unchanged
- [x] ☰ → `Keyboard Shortcuts` opens the same window; hovering ☰ reads `Menu`
...
- [x] With a screen and a real keyboard, on X11 (`make dev`): four accounts in
...
- [x] Three workspaces, one empty: `Ctrl`+`Tab` three times lands on the two
...
- [x] Holding `Ctrl`+`Tab` for two seconds switches exactly once; holding
      `Shift`+`Tab` steps exactly once
- [x] Sidebar selection mode: both keys change nothing and the game page does
      not receive them; leaving selection mode restores them
- [x] One account in the workspace: `Shift`+`Tab` changes nothing. One
      non-empty workspace: `Ctrl`+`Tab` changes nothing
- [x] The first Blocker: confirm `GtkWindow`'s bubble-phase `move-focus`
...
- [x] In the Windows VM (`scripts/windows-vm/compose.yml`) with a physical
      ...
      an empty one
- [x] In the VM, a page with a focused text field receives no `keydown` for
      either chord, and `Tab` alone still moves the field focus
- [x] In the VM, holding `Ctrl`+`Tab` for two seconds switches exactly once;
      the second Blocker (`GetKeyState(VK_SHIFT)` inside the accelerator
      callback) is confirmed
- [x] In the VM, `F5` and `Ctrl`+`+` still reload and zoom the focused game
      after the idle hop
...
- [x] Two accounts in `2`: the pager is absent; adding a third makes it
      appear reading `1/2`, directly left of the layout toggles
- [x] Clicking `›` on `1/2` shows the third account alone, its slot outlined
      and its sidebar row bold, readout `2/2`; clicking `›` again wraps to
      `1/2`; `‹` from `1/2` wraps to `2/2`
- [x] Switching from `2` to `4` with three accounts hides the pager; switching
      to `1` shows it reading `n/3` with the focused account on screen
- [x] Hovering the pager reads `Next account (Shift+Tab)`; the arrows read
      `Previous page` and `Next page`
- [x] Ten accounts in `Ungrouped` in `1`: stepping from `9/10` to `10/10`
      does not move the layout toggles
- [x] With the phone attached, clicking `›` changes the account the phone
      shows
...
- [x] Heading ⋯ shows `Park all` and `Start all` above `Rename…`;
      `Ungrouped`'s ⋯ shows only the two; each is greyed per the workspace's
      state; in selection mode no heading shows ⋯
- [x] `Park all` on a workspace with one live, one queued and one starting
      ...
      for it
- [x] `Start all` on three parked accounts brings them back one at a time in
      sidebar order: rows turn purple, then blue one at a time, then green
- [x] `Start all` during a still-draining launch restore appends to the
      queue: at no point are two accounts `Starting` at once
- [x] Hovering a workspace heading reads `Next workspace (Ctrl+Tab)`; the
...
- [x] In the Windows VM: the pager appears on the third account, turns and
...
- [x] In the VM: a drop onto the empty trailing slot of a part-empty last
      ...
      shortcuts window
- [x] In the VM: launching over a version 2 `sessions.toml` shows the page
...
- [x] Every unticked step above (all X11-manual and all Windows-VM steps) is
