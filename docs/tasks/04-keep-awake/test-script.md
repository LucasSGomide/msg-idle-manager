...
- [ ] Click the header bar's minimize button while a slotted account is running and read the same page's visibility — **not run.** `:99` has no window manager (none of openbox/icewm/xfwm4/fluxbox/twm/mutter/awesome/i3/bspwm/matchbox is installed, and none was installed to force one). The click reaches GTK and the button highlights, but nothing above GTK answers the iconify request: the window stays fully mapped and the page keeps reporting `visible`. No genuine minimised reading was taken.
